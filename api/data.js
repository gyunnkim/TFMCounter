...
export default async function handler(req, res) {
    ...
    try {
        if (req.method === 'GET') {
            // 데이터 조회
            const rawGameData = await kv.get(GAME_DATA_KEY);
            const gameDataFromKv = rawGameData || defaultGameData;
            const gameData = {
                ...defaultGameData,
                ...gameDataFromKv,
                selectedColonies: Array.isArray(gameDataFromKv?.selectedColonies) ? gameDataFromKv.selectedColonies : []
            };
